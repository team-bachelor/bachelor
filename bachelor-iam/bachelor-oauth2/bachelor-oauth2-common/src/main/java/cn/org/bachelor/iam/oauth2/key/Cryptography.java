...
//		}
//		// 准备key和iv
//		byte[] cn.org.bachelor.iam.oauth2.key = new byte[24];
//		byte[] iv = new byte[8];
//		// 根据上面混合的结果，生成key和iv
//		for (int i = 0; i < 32; i++) {
//			if (i < 24) {
//				cn.org.bachelor.iam.oauth2.key[i] = s[i];
//			} else {
//				iv[i - 24] = s[i];
//			}
//		}
...
//				.toString();
//		// 获取token1
//		byte[] token1 = TripleDES.encrypt(cn.org.bachelor.iam.oauth2.key, iv, src.getBytes());
//		// 取各种长度
//		byte keyLength = int2Byte(cn.org.bachelor.iam.oauth2.key.length);
//		byte ivLength = int2Byte(iv.length);
//		byte matrixLength = int2Byte(matrixSide);
//		// 计算token2的长度
//		// 数据长度
//		int dataLength = cn.org.bachelor.iam.oauth2.key.length + iv.length + token1.length + 5;// 5 = 前置数据的个数
//		// 补位长度
...
//		int start = 5;
//		// 加入key
//		appendByte(token2, cn.org.bachelor.iam.oauth2.key, start);
//		start += cn.org.bachelor.iam.oauth2.key.length;
//		// 加入token1
...
//		int start = 6;
//		byte[] cn.org.bachelor.iam.oauth2.key = readBytes(token2, start, keyLength);
//		start += keyLength;
//		byte[] token1 = readBytes(token2, start, token1Length);
//		start += token1Length;
//		byte[] iv = readBytes(token2, start, ivLength);
//		byte[] srcByte = TripleDES.decrypt(cn.org.bachelor.iam.oauth2.key, iv, token1);
...
//	}
	...
	public static void main(String[] arg) {
		...
		// }
		// }
		// System.out.println(cn.org.bachelor.iam.oauth2.key);
