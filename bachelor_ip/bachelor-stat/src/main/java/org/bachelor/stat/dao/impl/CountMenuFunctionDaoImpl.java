...
public class CountMenuFunctionDaoImpl extends 
	...
	public List countSystemRunEfficiency(CountConditionVo cc) {
		...
		qSQL.append("select (select m.name from t_bchlr_menu_info m  ");
		qSQL.append(" where m.function_id = up.id and rownum <2) as MenuName");
		qSQL.append(",round(avg(t.cost_time),0) CountNum from t_bchlr_statistic_efficiency t,t_bchlr_ps_function up ");
	...
	public void saveInfo(StatisticController sc) {
		...
		String cSQL = "insert into T_bchlr_STATISTIC_EFFICIENCY(id,REQUEST_PATH,COST_TIME,TYPE,ACCESS_DATE) values(";
