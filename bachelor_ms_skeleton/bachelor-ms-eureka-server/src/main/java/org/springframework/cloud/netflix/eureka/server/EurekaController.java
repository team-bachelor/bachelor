...
import java.util.*;
/**
 * Description: 重写EurekaController，添加识别apiUrl的方法，并在对应的status.ftl中加入链接入口
 * @Author Alexhendar
 * @Date: Created in 2018/11/1 11:40
 * @Modified By:
 */
@Controller
  ...
  private void populateApps(Map<String, Object> model) {
    ...
    for (Application app : sortedApplications) {
      ...
      for (InstanceInfo info : app.getInstances()) {
        ...
        apiUrl = info.getMetadata().get("api-url");
        if (!appData.containsKey("api-url")&&!StringUtils.isEmpty(apiUrl)) {
          appData.put("apiUrl", info.getHomePageUrl() + apiUrl);
        }
